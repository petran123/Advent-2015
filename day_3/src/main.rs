...
fn step_1() {
    ...
    let mut santa = Santa::start();

    //get the initial 0,0 visit
    santa.visit_house(&mut map);
    ...
    println!("first year, Santa alone visited {} houses!", map.keys().len());
}

fn step_2() {
    let mut shared_map = HashMap::<String, i32>::new();

    let mut santa = Santa::start();
    let mut robo_santa = Santa::start();

    //get the initial 0,0 visit
    santa.visit_house(&mut shared_map);

    INPUT.char_indices().for_each(|(step, char)| {
        let current_santa = if step % 2 == 1 { &mut santa } else { &mut robo_santa };
        current_santa.move_to(&char);
        current_santa.visit_house(&mut shared_map);
    });

    println!("Next year, Santa and Robo-Santa together visited {} houses!", shared_map.keys().len());
}
...
impl Santa {
    ...
    fn visit_house(&mut self, map: &mut HashMap<String, i32>) -> () {
        let key = self.x.to_string() + "," + &self.y.to_string();

        match map.get_mut(&key) {
            Some(prezzies) => *prezzies += 1,
            None => {
                map.insert(key, 1);
            }
        }
